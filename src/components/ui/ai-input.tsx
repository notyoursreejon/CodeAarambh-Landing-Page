...
export function MorphPanel({ onSubmit }: { onSubmit?: (prompt: string) => void }) {
  ...
  return (
    <div className="flex items-end justify-end pointer-events-none" style={{ width: FORM_WIDTH, height: FORM_HEIGHT }}>
      <motion.div
        ...
        data-panel
        className={cx(
          "bg-background pointer-events-auto relative z-3 flex flex-col items-center overflow-hidden border max-sm:bottom-5"
        )}
        initial={false}
        animate={{
    ...
    <footer className="mt-auto flex h-[44px] items-center justify-center whitespace-nowrap select-none">
      <div className="flex items-center justify-center gap-2 px-3 max-sm:h-10 max-sm:px-2">
        ...
        {!showForm && (
          <Button
            type="button"
            className="flex h-fit flex-1 justify-end rounded-full px-2 !py-0.5 text-zinc-400 hover:text-zinc-200"
            variant="ghost"
            onClick={triggerOpen}
          >
...
function InputForm({ ref, onSuccess }: { ref: React.Ref<HTMLTextAreaElement>; onSuccess: (prompt: string) => void }) {
